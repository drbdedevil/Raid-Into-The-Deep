...
public static class PathFinder
{
    ...
    public static List<Tile> FindTilesToChoose(Tile startTile, MapManager mapManager, int speed)
    {
        var result = new List<Tile>();
        Queue<TileCostPair> queue = new();
        queue.Enqueue(new TileCostPair(startTile, 0));
    ...
    public static List<Tile> FindTilesBySkill(BattleEntity skillOwner, Tile targetTile, MapManager mapManager, Tile startTile = null)
    {
        var result = new List<Tile>();
        if (skillOwner == null) return result;

        if (startTile is null) startTile = skillOwner.Tile;

        if (skillOwner is PlayerEntity playerSkillOwner)
        {
            var shapeTargets = playerSkillOwner.activeSkill.CalculateShapeAttackPositions(startTile.CartesianPosition, targetTile.CartesianPosition, mapManager);

            foreach (var shapeTarget in shapeTargets)
            {
                var skillTile = mapManager.GetTileByCartesianCoord(shapeTarget);
                if (skillTile is not null) result.Add(skillTile);
            }
        }
        return result;
    }
